...
class AuthErrorCodes:
    ...
    DISABLED = ErrorCode(
        code=1020006, description="第三方授权未启用", message_key="auth.disabled", http_status=503
    )
    ...
    CACHE = ErrorCode(
        code=1020008, description="授权缓存不可用", message_key="auth.cache", http_status=503
    )
    REJECTED = ErrorCode(
        code=1020009, description="第三方拒绝授权请求", message_key="auth.rejected"
    )
    RESPONSE = ErrorCode(
        code=1020010, description="第三方授权响应无效", message_key="auth.response", http_status=502
    )
    NETWORK = ErrorCode(
        code=1020011, description="第三方授权传输失败", message_key="auth.network", http_status=503
    )
    TIMEOUT = ErrorCode(
        code=1020012, description="第三方授权操作超时", message_key="auth.timeout", http_status=503
    )
